...
import 'package:store_app/screens/store_screens/views/user_list_view.dart';
...
class StoreHomeTab extends StatefulWidget {
  const StoreHomeTab({Key key, this.animationController}) : super(key: key);
  final AnimationController animationController;
  @override
  _StoreHomeTab createState() => _StoreHomeTab();
}

class _StoreHomeTab extends State<StoreHomeTab> with TickerProviderStateMixin {
  ...
  loadInvoices(BuildContext context) async {
    context.read<StoreProvider>().getUsers(context.read<AuthProvider>().token).then((res) async {
  ...
  _search(BuildContext context) async {
    final  _invoiceProvider =  context.read<StoreProvider>();
    if (_searchTextController.text.toString() == null || _searchTextController.text.toString().length == 0) {
      loadInvoices(context);
    }
    _invoiceProvider.fetchUsersByOptionsForStore( _searchTextController.text.trim(),context.read<AuthProvider>().token).then((res) async {
